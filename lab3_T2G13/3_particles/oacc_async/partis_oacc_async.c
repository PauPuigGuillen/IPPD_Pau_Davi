...
{
    #pragma acc parallel loop present(p_src[0:N], p_dst[0:N]) async(2)
    for (int i = 0; i < N; ++i)
    {
...
int main(int argc, char *argv[])
{
    ...
    while (t <= TOTAL_TIME)
    {
        ...
        if (iter % FREQ == 0)
        {
            ...
            // Wait for previous computation to complete before copying
            #pragma acc wait(1)
            copyFrame(pFrame, particles, N);

            if (write_flag)
            {
                // Wait for copy to complete before writing to disk
                #pragma acc wait(2)
    ...
    #pragma acc wait(1)
    #pragma acc wait(2)
    // Ensure final state is on host before validation
    ...
    if (write_flag)
    {
        // Read the last saved frame
        read_solution(pFrame, N, filename);
    }
    // Validate using the particles array which contains the final state
    int return_flag = validate(particles, N);
